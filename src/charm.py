...
def evaluate_status(func: Callable) -> Callable:
    """Decorate `InventoryExporterCharm` method to perform status evaluation.
    This wrapper can be used to decorate a method (primarily an event handler) from
    `InventoryExporterCharm` class. When decorated function is executed, this
    wrapper will perform final assessment of unit's state and sets unit status.
    """

    @wraps(func)
    def wrapper(self: "InventoryExporterCharm", *args: Any, **kwargs: Any) -> Any:
        ...
        if isinstance(self.unit.status, ActiveStatus) and not exporter_running:
            self.unit.status = BlockedStatus(
                "Inventory exporter service is inactive. (See service logs in unit.)"
            )
...
class InventoryExporterCharm(CharmBase):
    """Charm the service."""

    # Mapping between charm and snap configuration options
    SNAP_CONFIG_MAP = {
        'bind_address': 'exporter.addr',
        'port': 'exporter.port',
    }
    ...
    def snap_path(self) -> Optional[str]:
        ...
        if not self._snap_path_set:
            try:
                self._snap_path = str(self.model.resources.fetch("inventory-exporter"))
    ...
    def generate_exporter_config(self) -> Dict[str, Dict[str, Union[List[str], str, None]]]:
        ...
        config = ExporterConfig(
            ...
            address=self.config.get('bind_address'),
        )
    ...
    def _on_install(self, _: InstallEvent) -> None:
        """Install inventory-exporter snap."""
